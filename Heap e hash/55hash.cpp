...
void Hash::Inserir(const Funcionario& func) {
	unsigned rash = func.nascimento.Hash(mNroPosicoes);
	mElementos[rash].Adicionar(new Noh(func));
}
...
float Hash::PorcentagemDeUso() const {
	...
	return cont/float(mNroPosicoes);
}

unsigned Hash::QtdeAniversariantes(const Data& d) {
	unsigned rash = d.Hash(mNroPosicoes);
	Noh* nav = mElementos[rash].mPrimeiro;
	int cont = 0;
	while(nav != NULL ){
...
int main( ) {
    ...
    Hash tabela(qtde); // cria uma tabela com a quantidade passada de endereços
