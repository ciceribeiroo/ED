...
void Hash::Inserir(const Funcionario& func) {
   unsigned pos = func.nascimento.Hash(mNroPosicoes); //acha o rash, euquivalnte a posicao
   mElementos[pos].Adicionar(new Noh(func)); // adiciona na lista, ja com tratamento de erro 
   //( aniversarios na mesma data ), adicionando consequentemente na lista
}

float Hash::PorcentagemDeUso() const {
	float cont = 0.0; // cont pra fazer a intineração, float para possibilitar a amostragem de dados com virgula em return(169)
	for ( int i = 0; i < mNroPosicoes; i++ ) // percorre o vetor
	{
		if ( mElementos[i].mTamanho != 0 ) // se o tamanho da lista nao for zero, ou seja, se ela esta ocupada
		{
			cont++;
		}
	}
	return cont/float(mNroPosicoes); //retorna a porcentagem como decimal
}

unsigned Hash::QtdeAniversariantes(const Data& d) {
	unsigned pos = d.Hash(mNroPosicoes); // acha a posicao
	Noh* nav = mElementos[pos].mPrimeiro;
    int cont = 0;
    while ( nav!= NULL){
        if ( d.Dia() == nav->mValor.nascimento.Dia() and d.Mes() == nav->mValor.nascimento.Mes()) 
        {
			cont++;
		}
        nav = nav->mProximo;
