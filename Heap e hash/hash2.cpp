...
Hash::~Hash() {
    ...
    for (unsigned i = 0; i < mTamanho; i++){
        if(mVetPtDados[i] != REMOVIDO && mVetPtDados[i] != NULL){
...
unsigned Hash::Buscar(const std::string& chave) const {
    ...
    while( mVetPtDados[pos] != NULL && mVetPtDados[pos]->mChave != chave && cont < mCapacidade && encontrou == false){
	...
	if ( mVetPtDados[pos] != NULL && mVetPtDados[pos]->mChave == chave ){
		 return pos;
	}
	else {
		throw invalid_argument("Chave não compativel");
	}
}
