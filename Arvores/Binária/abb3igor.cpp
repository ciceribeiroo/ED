...
Arvore* Arvore::Menor() {
	Arvore* aux = this; //
	while (!aux->mPtEsq->mVazia)//enquanto a proxima sub arvore da esquerda não for vazia -- mesmo processo pra maior
	{
		aux = aux->mPtEsq;
	}
	return aux;
...
void Arvore::SubstituirPor(Arvore* ptArv) { //a antiga a raiz passa a ser um noh
    if (ptArv->mVazia)
        throw invalid_argument("Substitutição de nós falhou.");
    Arvore* antiga = this;
    Arvore* nova = new Arvore(*ptArv);//copia toda a sub_árvore de raiz ptArv (mudar?)
...
TValor& Arvore::Buscar(const TChave& chave) { //busca recursiva
    if (mVazia)
...
void Arvore::Inserir(const TChave& chave, const TValor& valor) {//inserir recursivo
